...
\begin{document}
	...
	\begin{abstract}
		...
		application-independent format for representing and exchanging educational
		questions and question banks. It provides a common question-content layer for
		the SciVault software suite and may also be implemented by third-party
		applications.
	...
	SciVault-QF version 0.1 defines both a standalone JSON document
	representation and a packaged question-set representation.
	
	A standalone SciVault-QF document uses:
	...
	Standalone JSON documents are suitable for development, validation, version
	control, interchange, and applications that do not require bundled media.
	
	A packaged SciVault-QF question set uses the package format defined in the
	following section. The package extension is \texttt{.sqf}.
	
	% ============================================================
	\section{SciVault-QF Package Format}
	% ============================================================
	
	A SciVault-QF package is a ZIP archive containing exactly one SciVault-QF
	question set together with any locally distributed resources required by
	that question set.
	
	\begin{itemize}
		\item \textbf{Extension:} \texttt{.sqf}
		\item \textbf{Container format:} ZIP
		\item \textbf{Text encoding:} UTF-8
		\item \textbf{Path separator:} \texttt{/}
	\end{itemize}
	
	The \texttt{.sqf} extension identifies the file as a SciVault-QF package.
	Applications shall interpret the file as a ZIP container conforming to the
	...
	A package containing local resources may contain:
	...
	The package shall contain exactly one SciVault-QF question set.
	
	The file \texttt{manifest.json} shall appear at the root of the archive.
	The SciVault-QF document referenced by the manifest shall also be contained
	within the archive.
	
	The \texttt{media/} directory is optional. Files and subdirectories within
	\texttt{media/} may be organized as appropriate for the question set.
	Applications shall use media references in the SciVault-QF document rather
	than infer resource meaning from directory or file names.
	
	% ------------------------------------------------------------
	\subsection{Package Manifest}
	% ------------------------------------------------------------
	
	The package manifest identifies the package format and the SciVault-QF
	document contained in the package.
	
	A version 0.1 manifest has the following form:
	...
	\begin{longtable}{|>{\ttfamily}p{3.0cm}|p{2.0cm}|p{1.7cm}|p{7.5cm}|}
		...
		Version of the SciVault-QF package format. \\ \hline
		
		content & String & Yes &
		Package-relative path to the SciVault-QF JSON document. \\ \hline
		
	\end{longtable}
	
	Version 0.1 packages should normally use \texttt{questions.json} as the
	content filename. Readers shall use the \texttt{content} field rather than
	assume a fixed filename.
	
	Question-set title, author, license, tags, and similar content metadata shall
	remain in the SciVault-QF document and shall not be duplicated in the package
	manifest.
	
	% ------------------------------------------------------------
	\subsection{Package Paths}
	% ------------------------------------------------------------
	
	Paths stored in a SciVault-QF package are relative to the package root.
	Forward slashes shall be used as path separators regardless of the host
	operating system.
	
	Package paths:
	
	\begin{itemize}
		\item shall not be absolute;
		\item shall not begin with \texttt{/};
		\item shall not contain a path component equal to \texttt{..}; and
		\item shall identify a unique archive entry.
	\end{itemize}
	
	Duplicate ZIP entry names are invalid.
	
	Path names are case-sensitive for purposes of SciVault-QF package
	resolution, even when a package is read on a case-insensitive file system.
	
	Readers that extract package contents shall prevent archive entries from
	being written outside the intended extraction directory.
	
	% ------------------------------------------------------------
	\subsection{Packaged Media}
	% ------------------------------------------------------------
	
	A local media reference in a packaged question set shall use a
	package-root-relative path.
	
	Example:
	
	\begin{lstlisting}
		"src": "media/images/force-diagram.svg"
	\end{lstlisting}
	
	This path refers to the archive entry:
	
	\begin{lstlisting}
		media/images/force-diagram.svg
	\end{lstlisting}
	
	Subdirectories within \texttt{media/} are permitted. Their organization is
	not semantically significant; the \texttt{src} field is the authoritative
	...
	\begin{lstlisting}
		{
			...
			"choices": [
			{"id": "A", "text": "$5\\,\\mathrm{m/s}$"},
			{"id": "B", "text": "$10\\,\\mathrm{m/s}$"},
			{"id": "C", "text": "$15\\,\\mathrm{m/s}$"},
			{"id": "D", "text": "$20\\,\\mathrm{m/s}$"}
			],
	...
	\begin{lstlisting}
		{
			"id": "q004",
			"type": "fill_in_the_blank",
			"question": "The SI unit of force is the {{blank}}.",
			"answer": ["newton", "Newton", "N"]
		}
	\end{lstlisting}
	...
	\begin{lstlisting}
		"hint": "Identify the initial velocity, acceleration, and elapsed time."
	\end{lstlisting}
	...
	\begin{lstlisting}
		"media": [
		{
			...
			"alt": "A block resting on a 30 degree inclined plane."
		}
		]
	\end{lstlisting}
	...
	Relative paths are recommended for locally distributed resources. In a
	SciVault-QF package, local \texttt{src} values shall be package-root-relative
	paths as defined by the SciVault-QF Package Format section.
	...
	\begin{lstlisting}
		"mu": {"type": "decimal", "min": 0.1, "max": 1.0, "step": 0.1}
	\end{lstlisting}
	...
	\begin{lstlisting}
		{
			...
			"question":
			"An object starts from rest and accelerates at "
			"{{a}} m/s^2 for {{t}} s. What is its final speed?",
			"variables": {
				"a": {"type": "integer", "min": 1, "max": 5},
				"t": {"type": "integer", "min": 2, "max": 10}
			},
			"answer": {
				"expression": "a * t",
				"tolerance": 0.01,
	...
	Application-specific data shall be placed inside an \texttt{extensions}
	object.
	...
	SciVault-QF document validation consists of:
	...
	Package validation additionally verifies the package manifest, archive paths,
	contained document, and local resource references.
	...
	\begin{longtable}{|>{\ttfamily}p{2.5cm}|p{11cm}|}
		...
		One or more errors make the document or package invalid. \\ \hline
	...
	Warnings do not make a document or package invalid.
	...
	\begin{lstlisting}
		{
			...
			"message": "Answer 'E' does not reference an existing choice.",
			"question_id": "q017",
	...
	\begin{longtable}{|>{\ttfamily}p{4cm}|p{9.5cm}|}
		\hline
		multiple\_choice & Exactly one correct answer selected from discrete choices. \\ \hline
		multiple\_select & One or more correct answers selected from discrete choices. \\ \hline
		true\_false & Binary true/false question. \\ \hline
		short\_answer & Short textual response. \\ \hline
		fill\_in\_the\_blank & One textual blank contained within a prompt. \\ \hline
		numerical & Numerical response with optional tolerance and physical unit. \\ \hline
	\end{longtable}
	...
	\begin{itemize}
		...
		\item localization and translated variants; and
	...
	\begin{lstlisting}
		{
			...
			"metadata": {
				"id": "org.scivault.physics.kinematics",
				"title": "Kinematics Concept Questions",
				"description": "Introductory kinematics questions.",
				"author": "Jane Doe",
				"license": "CC-BY-4.0",
				"version": "1.0",
				"language": "en-US",
			...
			"questions": [
			{
				...
				"answer": "B",
				"explanation":
				"Using $v=v_0+at$, $v=0+(2)(5)=10\\,\\mathrm{m/s}$.",
				"difficulty": 2,
				"tags": ["physics", "kinematics", "constant-acceleration"],
				"objectives": ["KIN-VELOCITY"],
				"settings": {"shuffle_choices": true}
			}
			]
		}
	\end{lstlisting}
	...
	A standalone document conforms to SciVault-QF 0.1 when:
	...
	A SciVault-QF package conforms to version 0.1 when:
	
	\begin{enumerate}
		\item it is a valid ZIP archive using the \texttt{.sqf} extension;
		\item it contains a valid root-level \texttt{manifest.json};
		\item the manifest identifies \texttt{SciVault-QF} version \texttt{0.1};
		\item the manifest references exactly one contained SciVault-QF document;
		\item the referenced document conforms to SciVault-QF 0.1;
		\item all required local media references resolve to valid package entries;
		\item package paths satisfy the package path requirements; and
		\item it contains no package-level semantic validation errors.
	\end{enumerate}
	...
	Version 0.1 supports standalone UTF-8 JSON documents using the standard
	\texttt{.json} extension and portable ZIP-based question-set packages using
	the \texttt{.sqf} extension.
	...
	SciVault-QF packages may contain one question set and its associated local
	resources. Future specifications may extend the question model and package
	capabilities while preserving portable interchange.
